...
object Day3MapNumbersNearSymbol extends App {

  def part1SumAdjacentToSymbol(input: Array[String]) = {
    sealed trait Element
    case class Number(n: Int) extends Element
    case object Symbol extends Element
    case object Point extends Element

    def charToElement(char: Char): Element = {
      char match {
        case '.' => Point
        case number if number >= '0' && number <= '9' => Number(number.toString.toInt)
        case _ => Symbol
      }
    }

    val mapOfElements: IndexedSeq[IndexedSeq[Element]] =
      input.toIndexedSeq.map { line =>
        line.toIndexedSeq.map { char =>
          charToElement(char)
        }
      }

    def isNextToSymbol(lineIndex: Int, charIndex: Int): Boolean = {
      mapOfElements.lift(lineIndex).exists(_.lift(charIndex).exists {
        case Symbol => true
        case _ => false
      })
    }
    ...
    val result =
      mapOfElements.zipWithIndex.map { case (line, lineIndex) =>
        var numberBuffer = ""
        var someNumberNextToSymbol = false

        val numbersToReturn: ArrayBuffer[Int] = ArrayBuffer[Int]()

        line.zipWithIndex.foreach { case (element, charIndex) =>
          element match {
            case Number(n) =>
              numberBuffer += n.toString
              val nextIsNumber = line.lift(charIndex + 1).exists {
                case _: Number => true
                case _ => false
              }
              val nextIsNotNumber = !nextIsNumber

              val nextToSymbol =
                isNextToSymbol(lineIndex, charIndex - 1) /*left*/ ||
                isNextToSymbol(lineIndex, charIndex + 1) /*right*/ ||
                isNextToSymbol(lineIndex + 1, charIndex) /*top*/ ||
                isNextToSymbol(lineIndex - 1, charIndex) /*bottom*/ ||
                isNextToSymbol(lineIndex + 1, charIndex - 1) /*diagonal left-top*/ ||
                isNextToSymbol(lineIndex - 1, charIndex - 1) /*diagonal left-bottom*/ ||
                isNextToSymbol(lineIndex + 1, charIndex + 1) /*diagonal right-top*/ ||
                isNextToSymbol(lineIndex - 1, charIndex + 1) /*diagonal right-bottom*/

              someNumberNextToSymbol = someNumberNextToSymbol || nextToSymbol

              if (nextIsNotNumber && someNumberNextToSymbol) {
                val fullNumber = numberBuffer.toInt
                numberBuffer = ""
                someNumberNextToSymbol = false
                numbersToReturn += fullNumber
              }
  ...
  def part2GearProductTwoNumbers(input: Array[String]) = {
    sealed trait Element
    case class Number(n: Int) extends Element
    case object Gear extends Element
    ...
    def isNextToGear(lineIndex: Int, charIndex: Int): Boolean = {
      mapOfElements.lift(lineIndex).exists(_.lift(charIndex).exists {
        case Gear => true
        case _ => false
      })
    ...
    case class FullNumberWithId(number: Int, id: Int)

    var numberI: Int = 0
    val numbersOnMap = mutable.HashMap.empty[(Int, Int), FullNumberWithId]

    // Set numbers on map
    mapOfElements.zipWithIndex.foreach { case (line, lineIndex) =>
      var numberBuffer = ""
      val numberBufferIndex = ArrayBuffer[Int]()
      var someNumberNextToSymbol = false
      ...
      line.zipWithIndex.foreach { case (element, charIndex) =>
        element match {
          case Number(n) =>
            ...
            val nextToGear =
              isNextToGear(lineIndex, charIndex - 1) /*left*/ ||
                isNextToGear(lineIndex, charIndex + 1) /*right*/ ||
                isNextToGear(lineIndex + 1, charIndex) /*top*/ ||
                isNextToGear(lineIndex - 1, charIndex) /*bottom*/ ||
                isNextToGear(lineIndex + 1, charIndex - 1) /*diagonal left-top*/ ||
                isNextToGear(lineIndex - 1, charIndex - 1) /*diagonal left-bottom*/ ||
                isNextToGear(lineIndex + 1, charIndex + 1) /*diagonal right-top*/ ||
                isNextToGear(lineIndex - 1, charIndex + 1) /*diagonal right-bottom*/

            someNumberNextToSymbol = someNumberNextToSymbol || nextToGear

            // Full number found
            if (nextIsNotNumber && someNumberNextToSymbol) {
              val fullNumber = numberBuffer.toInt
              numberBufferIndex.foreach { numberCharIndex =>
                numbersOnMap.update((lineIndex, numberCharIndex), FullNumberWithId(fullNumber, numberI))
              }
              numberI += 1

              // Clearing
              numberBuffer = ""
              numberBufferIndex.clear()
              someNumberNextToSymbol = false
            }
    ...
    def getNumber(lineIndex: Int, charIndex: Int): Option[FullNumberWithId] = {
      numbersOnMap.get((lineIndex, charIndex))
    }

    // Find numbers for each gear
    val result = mapOfElements.zipWithIndex.map { case (line, lineIndex) =>

      val productsToReturn = ArrayBuffer.empty[Int]

      line.zipWithIndex.foreach { case (element, charIndex) =>
        element match {
          case Gear =>

            val numbers: Seq[Option[FullNumberWithId]] =
              Seq(getNumber(lineIndex, charIndex - 1),
                getNumber(lineIndex, charIndex + 1),
                getNumber(lineIndex + 1, charIndex),
                getNumber(lineIndex - 1, charIndex),
                getNumber(lineIndex + 1, charIndex - 1),
                getNumber(lineIndex - 1, charIndex - 1),
                getNumber(lineIndex + 1, charIndex + 1),
                getNumber(lineIndex - 1, charIndex + 1)
              )
            val numbersWithoutOptions: Seq[FullNumberWithId] = numbers.flatten
            val groupById: Map[Int, Seq[FullNumberWithId]] = numbersWithoutOptions.groupBy(_.id)
            if (groupById.size == 2) {
              val onlyTheNumber: Seq[Int] = groupById.toSeq.map(_._2.head.number)
              productsToReturn += onlyTheNumber.product
            }
  ...
  assert(part1SumAdjacentToSymbol(Day3Input.input) == 4361)
  assert(part2GearProductTwoNumbers(Day3Input.input).flatten.sum == 467835)

  println(part2GearProductTwoNumbers(Day3Input.inputFull).flatten.sum)
...
object Day3Input {
  ...
  val inputFull = """...788.............................54.........501...........555.........270.................................521......893....................
                    ...
                    |....691............614...795..........152............120...........238..496...........................477..........................994......""".stripMargin.split('\n')
}
