...
object Day5 extends App {
  ...
  def part1(seeds: Array[Long], inputMap: Array[String]): Long = {

    case class MapFromTo(from: String, to: String) {
      def toFullName = s"$from-to-$to"
    }
    var currentMapFromTo: MapFromTo = MapFromTo("","")

    case class MapLine(source: Long, destination: Long, rangeLength: Long)

    val mapLines = ArrayBuffer.empty[MapLine]

    // HashMap(soil-to-fertilizer, ...)
    val maps = mutable.HashMap.empty[String, Seq[MapLine]]

    // HashMap(seed -> MapFromTo(seed,soil), soil -> MapFromTo(soil,fertilizer), ...)
    val srcToDst = mutable.HashMap.empty[String, MapFromTo]


    inputMap.foreach { line =>
      val mapHeaderPattern = "(\\w+)-to-(\\w+) map:".r
      val mapLinePattern = "(\\d+) (\\d+) (\\d+)".r

      line match {
        case mapHeaderPattern(from, to) =>
          currentMapFromTo = MapFromTo(from = from, to = to)
        case mapLinePattern(destination, source, rangeLength) =>
          mapLines += MapLine(source = source.toLong, destination = destination.toLong, rangeLength = rangeLength.toLong)
        case "" =>
          maps.put(currentMapFromTo.toFullName, mapLines.toSeq)
          srcToDst.put(currentMapFromTo.from, currentMapFromTo)
          mapLines.clear()
      }
    }
...
//    println(srcToDst)
    ...
    def inRange(number: Long, betweenLow: Long, betweenHigh: Long): Boolean = {
      number >= betweenLow && number <= betweenHigh
    }

    def findFinalDestination(mapName: String, sourceNumber: Long): Long = {
      val mapFromTo: MapFromTo = srcToDst(mapName)
      val mapLines: Seq[MapLine] = maps(mapFromTo.toFullName)
      val foundMapLine = mapLines.find { mapLine =>
        val includedSourceInRange = inRange(sourceNumber, betweenLow = mapLine.source, mapLine.source + mapLine.rangeLength)

        includedSourceInRange
      }.getOrElse(MapLine(source = sourceNumber, destination = sourceNumber, rangeLength = 1))
      val destination =
        sourceNumber + (foundMapLine.destination - foundMapLine.source)

      // Find the next
      srcToDst.get(mapFromTo.to) match {
        case Some(_) =>
          findFinalDestination(mapFromTo.to, destination)
        case None => destination
      }

    }

    val result = seeds.toSeq.map { seed =>
      findFinalDestination("seed", seed)
    }
    // Find the lowest location number
    result.min
  }

  def part2(seeds: Array[Long], inputMap: Array[String]): Long = {
    // Part 2 relies on part 1.
    // seeds: 79 14 55 13
    // two ranges:
    // 1. 79 length 14
    // 2. 55 length 13

    val mins = seeds.grouped(2).toSeq.par.map { groupOfTwo =>
      val startSeedNumber = groupOfTwo(0)
      val length = groupOfTwo(1)

      println(s"Start generating! $startSeedNumber $length")
      val seedBatches = (startSeedNumber until (startSeedNumber+length)).grouped(1000000)
      val min = seedBatches.map { seedBatch =>
        val seedBatchArray = seedBatch.toArray
        println(s"Getting the min! $startSeedNumber $length current: ${seedBatchArray(0)}")
        val part1Result = part1(seedBatchArray, inputMap)
        part1Result
      }.min


      println(s"Got the min! $min")
      min
    }
    ...
    assert(part1(Day5InputExample.inputSeeds, Day5InputExample.inputMap) == 35)

  assert(part1(Day5InputFull.inputSeeds, Day5InputFull.inputMap) == 600279879)

  assert(part2(Day5InputExample.inputSeeds, Day5InputExample.inputMap) == 46)

//  println(part2(Day5InputFull.inputSeeds, Day5InputFull.inputMap))
}

object Day5InputExample {

val inputSeeds: Array[Long] = "79 14 55 13".split(" ").map(_.toLong)

  //the destination range start, the source range start, and the range length

val inputMap = """seed-to-soil map:
  |50 98 2
  |52 50 48
  |
...
object Day5InputFull {

  val inputSeeds: Array[Long] = "929142010 467769747 2497466808 210166838 3768123711 33216796 1609270159 86969850 199555506 378609832 1840685500 314009711 1740069852 36868255 2161129344 170490105 2869967743 265455365 3984276455 31190888"
    .split(" ").map(_.toLong)
