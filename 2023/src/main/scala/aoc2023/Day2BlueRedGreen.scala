...
object Day2BlueRedGreen extends App {

  //12 red cubes, 13 green cubes, and 14 blue cubes
  def isGamePossible(game: Map[String, Int]): Boolean = {
    val red = game.get("red").getOrElse(0)
    val green = game.get("green").getOrElse(0)
    val blue = game.get("blue").getOrElse(0)

    red <= 12 && green <= 13 && blue <= 14
  }


  def maxOfEachColor(colorsWithNumber: Seq[String]): Map[String, Int] = {
    val hashmap = mutable.HashMap.empty[String, Int]

    colorsWithNumber.foreach { numberWithColor =>
      val pieces = numberWithColor.split(" ")
      val number = pieces(0).toInt
      val color = pieces(1)
      hashmap.update(color, Math.max(hashmap.get(color).getOrElse(0), number))
    }
  ...
  def task1() = {
    val res: Array[Int] = Day2Input.lines2.map { line =>
      val elements: Array[String] = line.split(':')
      val gameNumber: Int = elements(0).split(" ")(1).toInt
      val rest: String = elements(1)
      val colorsWithNumber: Seq[String] = rest.split(';').map(_.split(",").map(_.trim)).flatten.toSeq
      println(gameNumber)
      println(colorsWithNumber)
      val sumedColors = maxOfEachColor(colorsWithNumber)
      val isPossible = isGamePossible(sumedColors)
      printf(s"Game ${gameNumber} isGamePossible? $isPossible")
      if (isPossible) {
        gameNumber
      } else
        0
...
//      println(gameNumber)
//      println(colorsWithNumber)
      val sumedColors: Map[String, Int] = maxOfEachColor(colorsWithNumber)
      println(sumedColors)
      println(sumedColors.values.toSeq)

      val product = sumedColors.values.product

      println(s"Game ${gameNumber} product: ${product}")
      product
    }
    println("Result:" + res.sum)
  }
  ...
  task2()
  println(List(4, 2, 6).product)
}

object Day2Input {
  val lines: Array[String] = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
  ...
  val lines2 =
    """Game 1: 7 red, 8 blue; 6 blue, 6 red, 2 green; 2 red, 6 green, 8 blue; 9 green, 2 red, 4 blue; 6 blue, 4 green
      ...
      |Game 100: 10 red; 11 blue, 12 red; 1 green, 7 blue, 6 red""".stripMargin.split('\n')
}